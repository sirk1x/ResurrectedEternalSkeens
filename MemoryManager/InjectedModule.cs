...
namespace RedRain
{
	...
	public class InjectedModule : IDisposable
	{
		...
		/// <returns></returns>
		/// <exception cref="RedRain.Common.RedRainInjectionException">
		...
		public IntPtr GetExportPointer(string exportName)
		{
			...
			{
				if (lib == null)
					throw new RedRainInjectionException("Couldn't LoadLibrary into local thread to obtain export pointer!");

				var funcPtr = UnsafeNativeMethods.GetProcAddress(lib.DangerousGetHandle(), exportName);
				if (funcPtr == IntPtr.Zero)
					throw new RedRainInjectionException("Couldn't obtain function pointer for the specified export!");
		...
		/// <exception cref="RedRainException">
		...
		public IntPtr Call<T>(string exportName, T parameter, bool callWithParameter = false) where T : struct
		{
			...
			if (exportPtr == IntPtr.Zero)
				throw new RedRainException("Couldn't resolve export named with name " + exportName +
		...
		/// </returns>
		/// <exception cref="RedRainException">Couldn't find FreeMemory in Kernel32!</exception>
		/// <exception cref="RedRainInjectionException">
		///     WaitForSingleObject returned an unexpected value while waiting for the
		...
		public bool Free(bool isLocalProcessMemory)
		{
			...
			if (freeLibrary == IntPtr.Zero)
				throw new RedRainException("Couldn't find FreeMemory in Kernel32!");
