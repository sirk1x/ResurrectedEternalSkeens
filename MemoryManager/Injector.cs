...
namespace RedRain
{
	...
	public class Injector : IDisposable
	{
		...
		private InjectedModule InjectLibraryExternal(string libraryPath)
		{
			...
			try
			{
				...
				if (loadLibraryPtr == IntPtr.Zero)
					throw new RedRainInjectionException("Couldn't obtain handle to LoadLibraryW in remote process!");
				...
				using (var alloc = memory.Allocate((UIntPtr) pathBytes.Length))
				{
					...
					if (threadHandle.IsInvalid)
						throw new RedRainInjectionException(
				...
				if (!UnsafeNativeMethods.GetExitCodeThread(threadHandle.DangerousGetHandle(), out exitCode))
					throw new RedRainInjectionException("Couldn't obtain exit code for LoadLibraryW thread in remote process!");
		...
		private InjectedModule InjectLibraryInternal(string libraryPath)
		{
			...
			if (lib == null)
				throw new RedRainInjectionException("LoadLibrary failed in local process!");

			var module = Process.GetCurrentProcess().Modules.Cast<ProcessModule>().FirstOrDefault(s => s.FileName == libraryPath);

			if (module == null)
				throw new RedRainInjectionException("The injected library couldn't be found in the Process' module list!");
		...
		/// <returns></returns>
		/// <exception cref="RedRainException">Couldn't eject the specified library - it wasn't injected by this injector:  + path</exception>
		/// <exception cref="RedRainInjectionException">
		...
		public bool Eject(string path)
		{
			if (!InjectedModules.ContainsKey(path))
				throw new RedRainException("Couldn't eject the specified library - it wasn't injected by this injector: " + path);
